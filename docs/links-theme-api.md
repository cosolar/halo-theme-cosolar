...
当模板变量 `linkApplicationEnabled` 为 `true` 时，主题可以展示申请表单。页面底部的
"想在这里展示你的网站？"申请 CTA 区块可通过主题设置 `show_apply_cta` 开启或关闭。
适配需要使用以下两个同源端点：
...
主题在 `设置 → 友情链接页设置 → 申请表单字段显示` 中提供了开关，可控制以下非必填字段是否显示在申请表单中：

| 设置项         | 对应字段      | 说明     |
| -------------- | ------------- | -------- |
| 显示 Logo 地址 | `logo`        | 默认显示 |
| 显示联系邮箱   | `email`       | 默认显示 |
| 显示网站描述   | `description` | 默认显示 |
| 显示反链地址   | `backlink`    | 默认显示 |
| 显示订阅地址   | `feedUrls`    | 默认显示 |

必填字段 `url`、`displayName`、`captchaCode`、`_csrf` 始终显示，不可隐藏。
...
>
  ...
  <label>
    ...
    <textarea
      name="description"
      th:text="${param.field == 'description' ? param.value : ''}"
    ></textarea>
  </label>
  <label>
    联系邮箱
  ...
  <label>
    ...
    <textarea
      name="feedUrls"
      placeholder="每行一个"
      th:text="${param.field == 'feedUrls' ? param.value : ''}"
    ></textarea>
  </label>
...
```html
<th:block
  th:with="linkFeeds = ${linkFeedFinder.list({
  limit: 20
})}"
>
  <ul>
...
```html
linkFeedFinder.list({ limit: 20, linkName: 'link-name' })
```
...
| 对应模型 | group           | kind        | 主题扩展字段       |
| -------- | --------------- | ----------- | ------------------ |
| 链接     | `core.halo.run` | `Link`      | —                  |
| 链接分组 | `core.halo.run` | `LinkGroup` | `icon`（分组图标） |

### 分组图标（icon）

主题在「友情链接」页左侧的「资源分类」列表中展示链接分组，并为每个分组显示一个彩色渐变圆形图标。分组图标按以下优先级渲染：

1. **自定义图标**：在链接分组的元数据表单（`AnnotationSetting` 的 `LinkGroup` 表单，字段名 `icon`）中填写图标名。
   - Iconify 风格：填 `mdi:folder`、`mdi:tag` 等（可在 [icon-sets.iconify.design](https://icon-sets.iconify.design) 搜索），主题会以 `<iconify-icon>` 渲染。
   - iconfont 风格：填 `iconfont icon-xxx`，主题会以 `<i>` 字体图标渲染。
   - 图标色彩沿用该分组的彩色渐变圆底（蓝/绿/橙/紫/粉，按分组名映射），无需额外设置颜色。
2. **默认图标**：未填写 `icon` 时，使用主题内置的「文件夹/分类」图标（`mdi:folder` 风格），不再使用对号图标。

> 备注：圆形图标的颜色（蓝/绿/橙/紫/粉）仍由分组名（`技术`/`工具`/`资源`/`社区`/`生活`）映射决定，与是否设置自定义图标无关。
